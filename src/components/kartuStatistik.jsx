export function KartuStatistik({ statistics }) {
  const nextMedicine = statistics.next_medicine;
  let nextMedicineText = "-";

  if (nextMedicine && !nextMedicine.is_finished) {
    const hour = Math.floor(nextMedicine.remaining_minutes / 60);
    const minutes = nextMedicine.remaining_minutes % 60;

    if (hour > 0) {
      nextMedicineText = `${hour} jam`;
    } else {
      nextMedicineText = `${minutes} menit`;
    }
  }
  ...
  return (
    <div className="flex flex-wrap justify-center lg:grid lg:grid-cols-5 gap-5 mb-8">
      <div className="w-[calc(50%-10px)] lg:w-auto flex flex-col items-center rounded-2xl p-5 bg-white shadow-sm hover:shadow-md transition">
        ...
        <div className="font-bold text-2xl text-gray-900">{statistics.taken_today} / {statistics.total_today}</div>
      ...
      <div className="w-[calc(50%-10px)] lg:w-auto flex flex-col items-center rounded-2xl p-5 bg-white shadow-sm hover:shadow-md transition">
        ...
        <div className="font-bold text-2xl text-gray-900">{statistics.missed_this_week}</div>
      ...
      <div className="w-[calc(50%-10px)] lg:w-auto flex flex-col items-center rounded-2xl p-5 bg-white shadow-sm hover:shadow-md transition">
        ...
        <div className="font-bold text-2xl text-gray-900">{nextMedicineText}</div>
      ...
      <div className="w-[calc(50%-10px)] lg:w-auto  flex flex-col items-center rounded-2xl p-5 bg-white shadow-sm hover:shadow-md transition">
        ...
        <div className="font-bold text-2xl text-gray-900">{statistics.adherence.percentage}%</div>
